...
func newClient(cfg aws.Config, handlers request.Handlers, endpoint, signingRegion, signingName string) *EC2 {
	log.Printf("shengh.........EC2 newClient!", endpoint)
	svc := &EC2{
		Client: client.New(
			cfg,
			metadata.ClientInfo{
	...
	// Handlers
	log.Printf("shengh.........EC2 last SignRequestHandler !\n")
...
func (c *EC2) newRequest(op *request.Operation, params, data interface{}) *request.Request {
	req := c.NewRequest(op, params, data) //这里会调用client.go的NewRequest方法（继承）
	log.Printf("shengh.........EC2 newRequest!\n")
