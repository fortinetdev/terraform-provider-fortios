...
func flattenRouterRouteMapRule(v interface{}, d *schema.ResourceData, pre string) []map[string]interface{} {
	...
	for _, r := range l {
		...
		if _, ok := i["id"]; ok {
			tmp["id"] = flattenRouterRouteMapRuleId(i["id"], d, pre_append)
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "action"
		if _, ok := i["action"]; ok {
			tmp["action"] = flattenRouterRouteMapRuleAction(i["action"], d, pre_append)
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "match_as_path"
		if _, ok := i["match-as-path"]; ok {
			tmp["match_as_path"] = flattenRouterRouteMapRuleMatchAsPath(i["match-as-path"], d, pre_append)
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "match_community"
		if _, ok := i["match-community"]; ok {
			tmp["match_community"] = flattenRouterRouteMapRuleMatchCommunity(i["match-community"], d, pre_append)
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "match_community_exact"
		if _, ok := i["match-community-exact"]; ok {
			tmp["match_community_exact"] = flattenRouterRouteMapRuleMatchCommunityExact(i["match-community-exact"], d, pre_append)
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "match_origin"
		if _, ok := i["match-origin"]; ok {
			tmp["match_origin"] = flattenRouterRouteMapRuleMatchOrigin(i["match-origin"], d, pre_append)
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "match_interface"
		if _, ok := i["match-interface"]; ok {
			tmp["match_interface"] = flattenRouterRouteMapRuleMatchInterface(i["match-interface"], d, pre_append)
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "match_ip_address"
		if _, ok := i["match-ip-address"]; ok {
			tmp["match_ip_address"] = flattenRouterRouteMapRuleMatchIpAddress(i["match-ip-address"], d, pre_append)
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "match_ip6_address"
		if _, ok := i["match-ip6-address"]; ok {
			tmp["match_ip6_address"] = flattenRouterRouteMapRuleMatchIp6Address(i["match-ip6-address"], d, pre_append)
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "match_ip_nexthop"
		if _, ok := i["match-ip-nexthop"]; ok {
			tmp["match_ip_nexthop"] = flattenRouterRouteMapRuleMatchIpNexthop(i["match-ip-nexthop"], d, pre_append)
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "match_ip6_nexthop"
		if _, ok := i["match-ip6-nexthop"]; ok {
			tmp["match_ip6_nexthop"] = flattenRouterRouteMapRuleMatchIp6Nexthop(i["match-ip6-nexthop"], d, pre_append)
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "match_metric"
		if _, ok := i["match-metric"]; ok {
			tmp["match_metric"] = flattenRouterRouteMapRuleMatchMetric(i["match-metric"], d, pre_append)
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "match_route_type"
		if _, ok := i["match-route-type"]; ok {
			tmp["match_route_type"] = flattenRouterRouteMapRuleMatchRouteType(i["match-route-type"], d, pre_append)
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "match_tag"
		if _, ok := i["match-tag"]; ok {
			tmp["match_tag"] = flattenRouterRouteMapRuleMatchTag(i["match-tag"], d, pre_append)
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "set_aggregator_as"
		if _, ok := i["set-aggregator-as"]; ok {
			tmp["set_aggregator_as"] = flattenRouterRouteMapRuleSetAggregatorAs(i["set-aggregator-as"], d, pre_append)
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "set_aggregator_ip"
		if _, ok := i["set-aggregator-ip"]; ok {
			tmp["set_aggregator_ip"] = flattenRouterRouteMapRuleSetAggregatorIp(i["set-aggregator-ip"], d, pre_append)
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "set_aspath_action"
		if _, ok := i["set-aspath-action"]; ok {
			tmp["set_aspath_action"] = flattenRouterRouteMapRuleSetAspathAction(i["set-aspath-action"], d, pre_append)
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "set_aspath"
		if _, ok := i["set-aspath"]; ok {
			tmp["set_aspath"] = flattenRouterRouteMapRuleSetAspath(i["set-aspath"], d, pre_append)
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "set_atomic_aggregate"
		if _, ok := i["set-atomic-aggregate"]; ok {
			tmp["set_atomic_aggregate"] = flattenRouterRouteMapRuleSetAtomicAggregate(i["set-atomic-aggregate"], d, pre_append)
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "set_community_delete"
		if _, ok := i["set-community-delete"]; ok {
			tmp["set_community_delete"] = flattenRouterRouteMapRuleSetCommunityDelete(i["set-community-delete"], d, pre_append)
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "set_community"
		if _, ok := i["set-community"]; ok {
			tmp["set_community"] = flattenRouterRouteMapRuleSetCommunity(i["set-community"], d, pre_append)
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "set_community_additive"
		if _, ok := i["set-community-additive"]; ok {
			tmp["set_community_additive"] = flattenRouterRouteMapRuleSetCommunityAdditive(i["set-community-additive"], d, pre_append)
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "set_dampening_reachability_half_life"
		if _, ok := i["set-dampening-reachability-half-life"]; ok {
			tmp["set_dampening_reachability_half_life"] = flattenRouterRouteMapRuleSetDampeningReachabilityHalfLife(i["set-dampening-reachability-half-life"], d, pre_append)
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "set_dampening_reuse"
		if _, ok := i["set-dampening-reuse"]; ok {
			tmp["set_dampening_reuse"] = flattenRouterRouteMapRuleSetDampeningReuse(i["set-dampening-reuse"], d, pre_append)
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "set_dampening_suppress"
		if _, ok := i["set-dampening-suppress"]; ok {
			tmp["set_dampening_suppress"] = flattenRouterRouteMapRuleSetDampeningSuppress(i["set-dampening-suppress"], d, pre_append)
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "set_dampening_max_suppress"
		if _, ok := i["set-dampening-max-suppress"]; ok {
			tmp["set_dampening_max_suppress"] = flattenRouterRouteMapRuleSetDampeningMaxSuppress(i["set-dampening-max-suppress"], d, pre_append)
		}
		...
		if _, ok := i["set-dampening-unreachability-half-life"]; ok {
			tmp["set_dampening_unreachability_half_life"] = flattenRouterRouteMapRuleSetDampeningUnreachabilityHalfLife(i["set-dampening-unreachability-half-life"], d, pre_append)
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "set_extcommunity_rt"
		if _, ok := i["set-extcommunity-rt"]; ok {
			tmp["set_extcommunity_rt"] = flattenRouterRouteMapRuleSetExtcommunityRt(i["set-extcommunity-rt"], d, pre_append)
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "set_extcommunity_soo"
		if _, ok := i["set-extcommunity-soo"]; ok {
			tmp["set_extcommunity_soo"] = flattenRouterRouteMapRuleSetExtcommunitySoo(i["set-extcommunity-soo"], d, pre_append)
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "set_ip_nexthop"
		if _, ok := i["set-ip-nexthop"]; ok {
			tmp["set_ip_nexthop"] = flattenRouterRouteMapRuleSetIpNexthop(i["set-ip-nexthop"], d, pre_append)
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "set_ip6_nexthop"
		if _, ok := i["set-ip6-nexthop"]; ok {
			tmp["set_ip6_nexthop"] = flattenRouterRouteMapRuleSetIp6Nexthop(i["set-ip6-nexthop"], d, pre_append)
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "set_ip6_nexthop_local"
		if _, ok := i["set-ip6-nexthop-local"]; ok {
			tmp["set_ip6_nexthop_local"] = flattenRouterRouteMapRuleSetIp6NexthopLocal(i["set-ip6-nexthop-local"], d, pre_append)
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "set_local_preference"
		if _, ok := i["set-local-preference"]; ok {
			tmp["set_local_preference"] = flattenRouterRouteMapRuleSetLocalPreference(i["set-local-preference"], d, pre_append)
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "set_metric"
		if _, ok := i["set-metric"]; ok {
			tmp["set_metric"] = flattenRouterRouteMapRuleSetMetric(i["set-metric"], d, pre_append)
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "set_metric_type"
		if _, ok := i["set-metric-type"]; ok {
			tmp["set_metric_type"] = flattenRouterRouteMapRuleSetMetricType(i["set-metric-type"], d, pre_append)
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "set_originator_id"
		if _, ok := i["set-originator-id"]; ok {
			tmp["set_originator_id"] = flattenRouterRouteMapRuleSetOriginatorId(i["set-originator-id"], d, pre_append)
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "set_origin"
		if _, ok := i["set-origin"]; ok {
			tmp["set_origin"] = flattenRouterRouteMapRuleSetOrigin(i["set-origin"], d, pre_append)
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "set_tag"
		if _, ok := i["set-tag"]; ok {
			tmp["set_tag"] = flattenRouterRouteMapRuleSetTag(i["set-tag"], d, pre_append)
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "set_weight"
		if _, ok := i["set-weight"]; ok {
			tmp["set_weight"] = flattenRouterRouteMapRuleSetWeight(i["set-weight"], d, pre_append)
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "set_flags"
		if _, ok := i["set-flags"]; ok {
			tmp["set_flags"] = flattenRouterRouteMapRuleSetFlags(i["set-flags"], d, pre_append)
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "match_flags"
		if _, ok := i["match-flags"]; ok {
			tmp["match_flags"] = flattenRouterRouteMapRuleMatchFlags(i["match-flags"], d, pre_append)
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "set_route_tag"
		if _, ok := i["set-route-tag"]; ok {
			tmp["set_route_tag"] = flattenRouterRouteMapRuleSetRouteTag(i["set-route-tag"], d, pre_append)
		}
...
func flattenRouterRouteMapRuleSetAspath(v interface{}, d *schema.ResourceData, pre string) []map[string]interface{} {
	if v == nil {
		return nil
	}
	...
	for _, r := range l {
		...
		if _, ok := i["as"]; ok {
			tmp["as"] = flattenRouterRouteMapRuleSetAspathAs(i["as"], d, pre_append)
		}
...
func flattenRouterRouteMapRuleSetAspathAs(v interface{}, d *schema.ResourceData, pre string) interface{} {
	return v
}
...
func flattenRouterRouteMapRuleSetCommunity(v interface{}, d *schema.ResourceData, pre string) []map[string]interface{} {
	if v == nil {
		return nil
	}
	...
	for _, r := range l {
		...
		if _, ok := i["community"]; ok {
			tmp["community"] = flattenRouterRouteMapRuleSetCommunityCommunity(i["community"], d, pre_append)
		}
...
func flattenRouterRouteMapRuleSetCommunityCommunity(v interface{}, d *schema.ResourceData, pre string) interface{} {
	return v
}
...
func flattenRouterRouteMapRuleSetExtcommunityRt(v interface{}, d *schema.ResourceData, pre string) []map[string]interface{} {
	if v == nil {
		return nil
	}
	...
	for _, r := range l {
		...
		pre_append = pre + "." + strconv.Itoa(con) + "." + "community"
		if _, ok := i["community"]; ok {
			tmp["community"] = flattenRouterRouteMapRuleSetExtcommunityRtCommunity(i["community"], d, pre_append)
		}
...
func flattenRouterRouteMapRuleSetExtcommunityRtCommunity(v interface{}, d *schema.ResourceData, pre string) interface{} {
	return v
}

func flattenRouterRouteMapRuleSetExtcommunitySoo(v interface{}, d *schema.ResourceData, pre string) []map[string]interface{} {
	if v == nil {
		return nil
	}
	...
	for _, r := range l {
		...
		pre_append = pre + "." + strconv.Itoa(con) + "." + "community"
		if _, ok := i["community"]; ok {
			tmp["community"] = flattenRouterRouteMapRuleSetExtcommunitySooCommunity(i["community"], d, pre_append)
		}
...
func flattenRouterRouteMapRuleSetExtcommunitySooCommunity(v interface{}, d *schema.ResourceData, pre string) interface{} {
	return v
}
...
func expandRouterRouteMapRuleSetAspath(d *schema.ResourceData, v interface{}, pre string) (interface{}, error) {
	...
	for _, r := range l {
		...
		pre_append = pre + "." + strconv.Itoa(con) + "." + "as"
		if _, ok := d.GetOk(pre_append); ok {
			tmp["as"], _ = expandRouterRouteMapRuleSetAspathAs(d, i["as"], pre_append)
		}
...
func expandRouterRouteMapRuleSetAspathAs(d *schema.ResourceData, v interface{}, pre string) (interface{}, error) {
	return v, nil
}
...
func expandRouterRouteMapRuleSetCommunity(d *schema.ResourceData, v interface{}, pre string) (interface{}, error) {
	...
	for _, r := range l {
		...
		pre_append = pre + "." + strconv.Itoa(con) + "." + "community"
		if _, ok := d.GetOk(pre_append); ok {
			tmp["community"], _ = expandRouterRouteMapRuleSetCommunityCommunity(d, i["community"], pre_append)
		}
...
func expandRouterRouteMapRuleSetCommunityCommunity(d *schema.ResourceData, v interface{}, pre string) (interface{}, error) {
	return v, nil
}
...
func expandRouterRouteMapRuleSetExtcommunityRt(d *schema.ResourceData, v interface{}, pre string) (interface{}, error) {
	...
	for _, r := range l {
		...
		pre_append = pre + "." + strconv.Itoa(con) + "." + "community"
		if _, ok := d.GetOk(pre_append); ok {
			tmp["community"], _ = expandRouterRouteMapRuleSetExtcommunityRtCommunity(d, i["community"], pre_append)
		}
...
func expandRouterRouteMapRuleSetExtcommunityRtCommunity(d *schema.ResourceData, v interface{}, pre string) (interface{}, error) {
	return v, nil
}
...
func expandRouterRouteMapRuleSetExtcommunitySoo(d *schema.ResourceData, v interface{}, pre string) (interface{}, error) {
	...
	for _, r := range l {
		...
		pre_append = pre + "." + strconv.Itoa(con) + "." + "community"
		if _, ok := d.GetOk(pre_append); ok {
			tmp["community"], _ = expandRouterRouteMapRuleSetExtcommunitySooCommunity(d, i["community"], pre_append)
		}
...
func expandRouterRouteMapRuleSetExtcommunitySooCommunity(d *schema.ResourceData, v interface{}, pre string) (interface{}, error) {
	return v, nil
}
