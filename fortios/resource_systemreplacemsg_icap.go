...
func resourceSystemReplacemsgIcapUpdate(d *schema.ResourceData, m interface{}) error {
	...
	obj, err := getObjectSystemReplacemsgIcap(d, false, c.Fv)
	if err != nil {
...
func resourceSystemReplacemsgIcapDelete(d *schema.ResourceData, m interface{}) error {
	...
	obj, err := getObjectSystemReplacemsgIcap(d, true, c.Fv)

	if err != nil {
		return fmt.Errorf("Error updating SystemReplacemsgIcap resource while getting object: %v", err)
	}

	_, err = c.UpdateSystemReplacemsgIcap(obj, mkey, vdomparam)
	if err != nil {
		return fmt.Errorf("Error clearing SystemReplacemsgIcap resource: %v", err)
	}
...
func getObjectSystemReplacemsgIcap(d *schema.ResourceData, setArgNil bool, sv string) (*map[string]interface{}, error) {
