...
func flattenSystemMobileTunnelNMhaeKey(v interface{}, d *schema.ResourceData, pre string) interface{} {
	return v
}
...
func flattenSystemMobileTunnelNetwork(v interface{}, d *schema.ResourceData, pre string) []map[string]interface{} {
	...
	for _, r := range l {
		...
		if _, ok := i["id"]; ok {
			tmp["id"] = flattenSystemMobileTunnelNetworkId(i["id"], d, pre_append)
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "interface"
		if _, ok := i["interface"]; ok {
			tmp["interface"] = flattenSystemMobileTunnelNetworkInterface(i["interface"], d, pre_append)
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "prefix"
		if _, ok := i["prefix"]; ok {
			tmp["prefix"] = flattenSystemMobileTunnelNetworkPrefix(i["prefix"], d, pre_append)
		}
