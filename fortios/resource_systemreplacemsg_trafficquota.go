...
func resourceSystemReplacemsgTrafficQuotaUpdate(d *schema.ResourceData, m interface{}) error {
	...
	obj, err := getObjectSystemReplacemsgTrafficQuota(d, false, c.Fv)
	if err != nil {
...
func resourceSystemReplacemsgTrafficQuotaDelete(d *schema.ResourceData, m interface{}) error {
	...
	obj, err := getObjectSystemReplacemsgTrafficQuota(d, true, c.Fv)

	if err != nil {
		return fmt.Errorf("Error updating SystemReplacemsgTrafficQuota resource while getting object: %v", err)
	}

	_, err = c.UpdateSystemReplacemsgTrafficQuota(obj, mkey, vdomparam)
	if err != nil {
		return fmt.Errorf("Error clearing SystemReplacemsgTrafficQuota resource: %v", err)
	}
...
func getObjectSystemReplacemsgTrafficQuota(d *schema.ResourceData, setArgNil bool, sv string) (*map[string]interface{}, error) {
