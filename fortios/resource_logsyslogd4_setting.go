...
func flattenLogSyslogd4SettingVrfSelect(v interface{}, d *schema.ResourceData, pre string, sv string) interface{} {
	return convintf2i(v)
}

func flattenLogSyslogd4SettingCustomLogFormat(v interface{}, d *schema.ResourceData, pre string, sv string) interface{} {
	return v
}
...
func refreshObjectLogSyslogd4Setting(d *schema.ResourceData, o map[string]interface{}, sv string) error {
	...
	if err = d.Set("vrf_select", flattenLogSyslogd4SettingVrfSelect(o["vrf-select"], d, "vrf_select", sv)); err != nil {
		if !fortiAPIPatch(o["vrf-select"]) {
			return fmt.Errorf("Error reading vrf_select: %v", err)
		}
	}

	if err = d.Set("custom_log_format", flattenLogSyslogd4SettingCustomLogFormat(o["custom-log-format"], d, "custom_log_format", sv)); err != nil {
...
func expandLogSyslogd4SettingVrfSelect(d *schema.ResourceData, v interface{}, pre string, sv string) (interface{}, error) {
	return v, nil
}

func expandLogSyslogd4SettingCustomLogFormat(d *schema.ResourceData, v interface{}, pre string, sv string) (interface{}, error) {
	return v, nil
}
...
func getObjectLogSyslogd4Setting(d *schema.ResourceData, setArgNil bool, sv string) (*map[string]interface{}, error) {
	...
	if v, ok := d.GetOkExists("vrf_select"); ok {
		if setArgNil {
			...
			t, err := expandLogSyslogd4SettingVrfSelect(d, v, "vrf_select", sv)
			if err != nil {
				return &obj, err
			} else if t != nil {
				obj["vrf-select"] = t
	...
	if v, ok := d.GetOk("custom_log_format"); ok {
		if setArgNil {
			...
			t, err := expandLogSyslogd4SettingCustomLogFormat(d, v, "custom_log_format", sv)
			if err != nil {
				return &obj, err
			} else if t != nil {
