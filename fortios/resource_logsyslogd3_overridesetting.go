...
func flattenLogSyslogd3OverrideSettingCustomFieldName(v interface{}, d *schema.ResourceData, pre string) []map[string]interface{} {
	...
	for _, r := range l {
		...
		if _, ok := i["id"]; ok {
			tmp["id"] = flattenLogSyslogd3OverrideSettingCustomFieldNameId(i["id"], d, pre_append)
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "name"
		if _, ok := i["name"]; ok {
			tmp["name"] = flattenLogSyslogd3OverrideSettingCustomFieldNameName(i["name"], d, pre_append)
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "custom"
		if _, ok := i["custom"]; ok {
			tmp["custom"] = flattenLogSyslogd3OverrideSettingCustomFieldNameCustom(i["custom"], d, pre_append)
		}
...
func flattenLogSyslogd3OverrideSettingSyslogType(v interface{}, d *schema.ResourceData, pre string) interface{} {
	return v
}
...
func refreshObjectLogSyslogd3OverrideSetting(d *schema.ResourceData, o map[string]interface{}) error {
	...
	if err = d.Set("syslog_type", flattenLogSyslogd3OverrideSettingSyslogType(o["syslog-type"], d, "syslog_type")); err != nil {
...
func expandLogSyslogd3OverrideSettingSyslogType(d *schema.ResourceData, v interface{}, pre string) (interface{}, error) {
	return v, nil
}
...
func getObjectLogSyslogd3OverrideSetting(d *schema.ResourceData) (*map[string]interface{}, error) {
	...
	if v, ok := d.GetOkExists("syslog_type"); ok {
		t, err := expandLogSyslogd3OverrideSettingSyslogType(d, v, "syslog_type")
		if err != nil {
			return &obj, err
		} else if t != nil {
			obj["syslog-type"] = t
