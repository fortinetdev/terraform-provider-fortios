...
func flattenFirewallVipSrcFilter(v interface{}, d *schema.ResourceData, pre string) []map[string]interface{} {
	if v == nil {
		return nil
	}
	...
	for _, r := range l {
		...
		if _, ok := i["range"]; ok {
			tmp["range"] = flattenFirewallVipSrcFilterRange(i["range"], d, pre_append)
		}
...
func flattenFirewallVipSrcFilterRange(v interface{}, d *schema.ResourceData, pre string) interface{} {
	return v
}

func flattenFirewallVipService(v interface{}, d *schema.ResourceData, pre string) []map[string]interface{} {
	if v == nil {
		return nil
	}
	...
	for _, r := range l {
		...
		if _, ok := i["name"]; ok {
			tmp["name"] = flattenFirewallVipServiceName(i["name"], d, pre_append)
		}
...
func flattenFirewallVipServiceName(v interface{}, d *schema.ResourceData, pre string) interface{} {
	return v
}
...
func flattenFirewallVipExtaddr(v interface{}, d *schema.ResourceData, pre string) []map[string]interface{} {
	if v == nil {
		return nil
	}
	...
	for _, r := range l {
		...
		pre_append = pre + "." + strconv.Itoa(con) + "." + "name"
		if _, ok := i["name"]; ok {
			tmp["name"] = flattenFirewallVipExtaddrName(i["name"], d, pre_append)
		}
...
func flattenFirewallVipExtaddrName(v interface{}, d *schema.ResourceData, pre string) interface{} {
	return v
}

func flattenFirewallVipMappedip(v interface{}, d *schema.ResourceData, pre string) []map[string]interface{} {
	if v == nil {
		return nil
	}
	...
	for _, r := range l {
		...
		pre_append = pre + "." + strconv.Itoa(con) + "." + "range"
		if _, ok := i["range"]; ok {
			tmp["range"] = flattenFirewallVipMappedipRange(i["range"], d, pre_append)
		}
...
func flattenFirewallVipMappedipRange(v interface{}, d *schema.ResourceData, pre string) interface{} {
	return v
}
...
func flattenFirewallVipSrcintfFilter(v interface{}, d *schema.ResourceData, pre string) []map[string]interface{} {
	if v == nil {
		return nil
	}
	...
	for _, r := range l {
		...
		if _, ok := i["interface-name"]; ok {
			tmp["interface_name"] = flattenFirewallVipSrcintfFilterInterfaceName(i["interface-name"], d, pre_append)
		}
...
func flattenFirewallVipSrcintfFilterInterfaceName(v interface{}, d *schema.ResourceData, pre string) interface{} {
	return v
}
...
func flattenFirewallVipRealservers(v interface{}, d *schema.ResourceData, pre string) []map[string]interface{} {
	...
	for _, r := range l {
		...
		if _, ok := i["id"]; ok {
			tmp["id"] = flattenFirewallVipRealserversId(i["id"], d, pre_append)
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "ip"
		if _, ok := i["ip"]; ok {
			tmp["ip"] = flattenFirewallVipRealserversIp(i["ip"], d, pre_append)
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "port"
		if _, ok := i["port"]; ok {
			tmp["port"] = flattenFirewallVipRealserversPort(i["port"], d, pre_append)
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "status"
		if _, ok := i["status"]; ok {
			tmp["status"] = flattenFirewallVipRealserversStatus(i["status"], d, pre_append)
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "weight"
		if _, ok := i["weight"]; ok {
			tmp["weight"] = flattenFirewallVipRealserversWeight(i["weight"], d, pre_append)
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "holddown_interval"
		if _, ok := i["holddown-interval"]; ok {
			tmp["holddown_interval"] = flattenFirewallVipRealserversHolddownInterval(i["holddown-interval"], d, pre_append)
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "healthcheck"
		if _, ok := i["healthcheck"]; ok {
			tmp["healthcheck"] = flattenFirewallVipRealserversHealthcheck(i["healthcheck"], d, pre_append)
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "http_host"
		if _, ok := i["http-host"]; ok {
			tmp["http_host"] = flattenFirewallVipRealserversHttpHost(i["http-host"], d, pre_append)
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "max_connections"
		if _, ok := i["max-connections"]; ok {
			tmp["max_connections"] = flattenFirewallVipRealserversMaxConnections(i["max-connections"], d, pre_append)
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "monitor"
		if _, ok := i["monitor"]; ok {
			tmp["monitor"] = flattenFirewallVipRealserversMonitor(i["monitor"], d, pre_append)
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "client_ip"
		if _, ok := i["client-ip"]; ok {
			tmp["client_ip"] = flattenFirewallVipRealserversClientIp(i["client-ip"], d, pre_append)
		}
...
func flattenFirewallVipSslCipherSuites(v interface{}, d *schema.ResourceData, pre string) []map[string]interface{} {
	...
	for _, r := range l {
		...
		if _, ok := i["priority"]; ok {
			tmp["priority"] = flattenFirewallVipSslCipherSuitesPriority(i["priority"], d, pre_append)
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "cipher"
		if _, ok := i["cipher"]; ok {
			tmp["cipher"] = flattenFirewallVipSslCipherSuitesCipher(i["cipher"], d, pre_append)
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "versions"
		if _, ok := i["versions"]; ok {
			tmp["versions"] = flattenFirewallVipSslCipherSuitesVersions(i["versions"], d, pre_append)
		}
...
func flattenFirewallVipSslServerCipherSuites(v interface{}, d *schema.ResourceData, pre string) []map[string]interface{} {
	...
	for _, r := range l {
		...
		pre_append = pre + "." + strconv.Itoa(con) + "." + "priority"
		if _, ok := i["priority"]; ok {
			tmp["priority"] = flattenFirewallVipSslServerCipherSuitesPriority(i["priority"], d, pre_append)
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "cipher"
		if _, ok := i["cipher"]; ok {
			tmp["cipher"] = flattenFirewallVipSslServerCipherSuitesCipher(i["cipher"], d, pre_append)
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "versions"
		if _, ok := i["versions"]; ok {
			tmp["versions"] = flattenFirewallVipSslServerCipherSuitesVersions(i["versions"], d, pre_append)
		}
...
func flattenFirewallVipMonitor(v interface{}, d *schema.ResourceData, pre string) []map[string]interface{} {
	if v == nil {
		return nil
	}
	...
	for _, r := range l {
		...
		pre_append = pre + "." + strconv.Itoa(con) + "." + "name"
		if _, ok := i["name"]; ok {
			tmp["name"] = flattenFirewallVipMonitorName(i["name"], d, pre_append)
		}
...
func flattenFirewallVipMonitorName(v interface{}, d *schema.ResourceData, pre string) interface{} {
	return v
}
...
func expandFirewallVipSrcFilter(d *schema.ResourceData, v interface{}, pre string) (interface{}, error) {
	...
	for _, r := range l {
		...
		if _, ok := d.GetOk(pre_append); ok {
			tmp["range"], _ = expandFirewallVipSrcFilterRange(d, i["range"], pre_append)
		}
...
func expandFirewallVipSrcFilterRange(d *schema.ResourceData, v interface{}, pre string) (interface{}, error) {
	return v, nil
}
...
func expandFirewallVipService(d *schema.ResourceData, v interface{}, pre string) (interface{}, error) {
	...
	for _, r := range l {
		...
		pre_append = pre + "." + strconv.Itoa(con) + "." + "name"
		if _, ok := d.GetOk(pre_append); ok {
			tmp["name"], _ = expandFirewallVipServiceName(d, i["name"], pre_append)
		}
...
func expandFirewallVipServiceName(d *schema.ResourceData, v interface{}, pre string) (interface{}, error) {
	return v, nil
}
...
func expandFirewallVipExtaddr(d *schema.ResourceData, v interface{}, pre string) (interface{}, error) {
	...
	for _, r := range l {
		...
		pre_append = pre + "." + strconv.Itoa(con) + "." + "name"
		if _, ok := d.GetOk(pre_append); ok {
			tmp["name"], _ = expandFirewallVipExtaddrName(d, i["name"], pre_append)
		}
...
func expandFirewallVipExtaddrName(d *schema.ResourceData, v interface{}, pre string) (interface{}, error) {
	return v, nil
}
...
func expandFirewallVipMappedip(d *schema.ResourceData, v interface{}, pre string) (interface{}, error) {
	...
	for _, r := range l {
		...
		pre_append = pre + "." + strconv.Itoa(con) + "." + "range"
		if _, ok := d.GetOk(pre_append); ok {
			tmp["range"], _ = expandFirewallVipMappedipRange(d, i["range"], pre_append)
		}
...
func expandFirewallVipMappedipRange(d *schema.ResourceData, v interface{}, pre string) (interface{}, error) {
	return v, nil
}
...
func expandFirewallVipSrcintfFilter(d *schema.ResourceData, v interface{}, pre string) (interface{}, error) {
	...
	for _, r := range l {
		...
		pre_append = pre + "." + strconv.Itoa(con) + "." + "interface_name"
		if _, ok := d.GetOk(pre_append); ok {
			tmp["interface-name"], _ = expandFirewallVipSrcintfFilterInterfaceName(d, i["interface_name"], pre_append)
		}
...
func expandFirewallVipSrcintfFilterInterfaceName(d *schema.ResourceData, v interface{}, pre string) (interface{}, error) {
	return v, nil
}
...
func expandFirewallVipMonitor(d *schema.ResourceData, v interface{}, pre string) (interface{}, error) {
	...
	for _, r := range l {
		...
		pre_append = pre + "." + strconv.Itoa(con) + "." + "name"
		if _, ok := d.GetOk(pre_append); ok {
			tmp["name"], _ = expandFirewallVipMonitorName(d, i["name"], pre_append)
		}
...
func expandFirewallVipMonitorName(d *schema.ResourceData, v interface{}, pre string) (interface{}, error) {
	return v, nil
}
