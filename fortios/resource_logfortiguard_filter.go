...
func resourceLogFortiguardFilterUpdate(d *schema.ResourceData, m interface{}) error {
	...
	obj, err := getObjectLogFortiguardFilter(d, false, c.Fv)
	if err != nil {
		return fmt.Errorf("Error updating LogFortiguardFilter resource while getting object: %v", err)
	}

	o, err := c.UpdateLogFortiguardFilter(obj, mkey, vdomparam)
	if err != nil {
...
func resourceLogFortiguardFilterDelete(d *schema.ResourceData, m interface{}) error {
	...
	obj, err := getObjectLogFortiguardFilter(d, true, c.Fv)

	if err != nil {
		return fmt.Errorf("Error updating LogFortiguardFilter resource while getting object: %v", err)
	}

	_, err = c.UpdateLogFortiguardFilter(obj, mkey, vdomparam)
	if err != nil {
...
func resourceLogFortiguardFilterRead(d *schema.ResourceData, m interface{}) error {
	...
	o, err := c.ReadLogFortiguardFilter(mkey, vdomparam)
	if err != nil {
...
func flattenLogFortiguardFilterFortiSwitch(v interface{}, d *schema.ResourceData, pre string, sv string) interface{} {
	return v
}
...
func flattenLogFortiguardFilterFreeStyle(v interface{}, d *schema.ResourceData, pre string, sv string) []map[string]interface{} {
	...
	for _, r := range parsed_list {
		...
		if cur_v, ok := i["id"]; ok {
			...
			tmp["id"] = flattenLogFortiguardFilterFreeStyleId(cur_v, d, pre_append, sv)
		}
		...
		if cur_v, ok := i["category"]; ok {
			...
			tmp["category"] = flattenLogFortiguardFilterFreeStyleCategory(cur_v, d, pre_append, sv)
		}
		...
		if cur_v, ok := i["filter"]; ok {
			...
			tmp["filter"] = flattenLogFortiguardFilterFreeStyleFilter(cur_v, d, pre_append, sv)
		}
		...
		if cur_v, ok := i["filter-type"]; ok {
			...
			tmp["filter_type"] = flattenLogFortiguardFilterFreeStyleFilterType(cur_v, d, pre_append, sv)
		}
...
func refreshObjectLogFortiguardFilter(d *schema.ResourceData, o map[string]interface{}, sv string) error {
	...
	if err = d.Set("forti_switch", flattenLogFortiguardFilterFortiSwitch(o["forti-switch"], d, "forti_switch", sv)); err != nil {
...
func getObjectLogFortiguardFilter(d *schema.ResourceData, setArgNil bool, sv string) (*map[string]interface{}, error) {
	...
	if v, ok := d.GetOk("forti_switch"); ok {
		if setArgNil {
			...
			t, err := expandLogFortiguardFilterFortiSwitch(d, v, "forti_switch", sv)
			if err != nil {
				return &obj, err
			} else if t != nil {
				obj["forti-switch"] = t
