...
func resourceSystemReplacemsgHttpUpdate(d *schema.ResourceData, m interface{}) error {
	...
	obj, err := getObjectSystemReplacemsgHttp(d, false, c.Fv)
	if err != nil {
...
func resourceSystemReplacemsgHttpDelete(d *schema.ResourceData, m interface{}) error {
	...
	obj, err := getObjectSystemReplacemsgHttp(d, true, c.Fv)

	if err != nil {
		return fmt.Errorf("Error updating SystemReplacemsgHttp resource while getting object: %v", err)
	}

	_, err = c.UpdateSystemReplacemsgHttp(obj, mkey, vdomparam)
	if err != nil {
		return fmt.Errorf("Error clearing SystemReplacemsgHttp resource: %v", err)
	}
...
func getObjectSystemReplacemsgHttp(d *schema.ResourceData, setArgNil bool, sv string) (*map[string]interface{}, error) {
