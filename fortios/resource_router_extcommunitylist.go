...
func resourceRouterExtcommunityListCreate(d *schema.ResourceData, m interface{}) error {
	...
	return readBackAfterCreate(d, "RouterExtcommunityList", mkey,
		func() (map[string]interface{}, error) { return c.ReadRouterExtcommunityList(mkey, vdomparam) },
		refreshObjectRouterExtcommunityList, c.Fv)
}
...
func flattenRouterExtcommunityListRule(v interface{}, d *schema.ResourceData, pre string, sv string) []map[string]interface{} {
	...
	for _, r := range parsed_list {
		...
		if cur_v, ok := i["id"]; ok {
			...
			tmp["id"] = flattenRouterExtcommunityListRuleId(cur_v, d, pre_append, sv)
		}
		...
		if cur_v, ok := i["action"]; ok {
			...
			tmp["action"] = flattenRouterExtcommunityListRuleAction(cur_v, d, pre_append, sv)
		}
		...
		if cur_v, ok := i["regexp"]; ok {
			...
			tmp["regexp"] = flattenRouterExtcommunityListRuleRegexp(cur_v, d, pre_append, sv)
		}
		...
		if cur_v, ok := i["type"]; ok {
			...
			tmp["type"] = flattenRouterExtcommunityListRuleType(cur_v, d, pre_append, sv)
		}
		...
		if cur_v, ok := i["match"]; ok {
			...
			tmp["match"] = flattenRouterExtcommunityListRuleMatch(cur_v, d, pre_append, sv)
		}
