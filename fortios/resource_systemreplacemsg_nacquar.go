...
func resourceSystemReplacemsgNacQuarUpdate(d *schema.ResourceData, m interface{}) error {
	...
	obj, err := getObjectSystemReplacemsgNacQuar(d, false, c.Fv)
	if err != nil {
...
func resourceSystemReplacemsgNacQuarDelete(d *schema.ResourceData, m interface{}) error {
	...
	obj, err := getObjectSystemReplacemsgNacQuar(d, true, c.Fv)

	if err != nil {
		return fmt.Errorf("Error updating SystemReplacemsgNacQuar resource while getting object: %v", err)
	}

	_, err = c.UpdateSystemReplacemsgNacQuar(obj, mkey, vdomparam)
	if err != nil {
		return fmt.Errorf("Error clearing SystemReplacemsgNacQuar resource: %v", err)
	}
...
func getObjectSystemReplacemsgNacQuar(d *schema.ResourceData, setArgNil bool, sv string) (*map[string]interface{}, error) {
