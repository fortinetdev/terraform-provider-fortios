...
* `minimum_length` - Minimum password length. On FortiOS versions 6.2.0-7.6.4: (8 - 128, default = 8). On FortiOS versions >= 7.6.5: (12 - 128, default = 12).
* `min_lower_case_letter` - Minimum number of lowercase characters in password (0 - 128). On FortiOS versions 6.2.0-7.6.4: default = 0. On FortiOS versions >= 7.6.5: default = 1.
* `min_upper_case_letter` - Minimum number of uppercase characters in password (0 - 128). On FortiOS versions 6.2.0-7.6.4: default = 0. On FortiOS versions >= 7.6.5: default = 1.
* `min_non_alphanumeric` - Minimum number of non-alphanumeric characters in password (0 - 128). On FortiOS versions 6.2.0-7.6.4: default = 0. On FortiOS versions >= 7.6.5: default = 1.
* `min_number` - Minimum number of numeric characters in password (0 - 128). On FortiOS versions 6.2.0-7.6.4: default = 0. On FortiOS versions >= 7.6.5: default = 1.
...
* `login_lockout_upon_weaker_encryption` - Enable/disable administrative user login lockout upon downgrade (default = disable). If enabled, changing the FortiOS firmware to a version where safer passwords are unsupported will lock out administrative users. Valid values: `enable`, `disable`.
