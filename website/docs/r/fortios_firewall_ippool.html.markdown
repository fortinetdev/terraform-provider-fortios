...
* `source_startip` - (Required)  First IPv4 address (inclusive) in the range of the source addresses to be translated (format xxx.xxx.xxx.xxx, Default: 0.0.0.0).
* `source_endip` - (Required) Final IPv4 address (inclusive) in the range of the source addresses to be translated (format xxx.xxx.xxx.xxx, Default: 0.0.0.0).
* `block_size` - (Required)  Number of addresses in a block (64 to 4096, default = 128).
* `num_blocks_per_user` - (Required) Number of addresses blocks that can be used by a user (1 to 128, default = 8).
