...
layout: "fortios"
page_title: "FortiOS: fortios_telemetrycontroller_agentprofile"
description: |-
  Configure FortiTelemetry agent profiles.
---

# fortios_telemetrycontroller_agentprofile
Configure FortiTelemetry agent profiles. Applies to FortiOS Version `>= 8.0.0`.
...
* `comment` - Comment.
* `model` - Model. Valid values: `ftl-100g`, `windows`, `macos`.
...
TelemetryController AgentProfile can be imported using any of these accepted formats:
```
$ terraform import fortios_telemetrycontroller_agentprofile.labelname {{name}}
