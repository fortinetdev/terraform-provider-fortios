...
* `monitor_prefix` - Configure a list of routing prefixes to monitor. The structure of `monitor_prefix` block is documented below.
* `helper_traffic_bounce` - Enable/disable helper related traffic bounce. Valid values: `enable`, `disable`.
* `utm_traffic_bounce` - Enable/disable UTM related traffic bounce. Valid values: `enable`, `disable`.
